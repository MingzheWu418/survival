# https://github.com/luisvalesilva/multisurv/blob/master/src/evaluation.py
...
class _BaselineModelEvaluation(_BaseEvaluation):
    ...
    def _predict(self, time_points=None): # changed
        ...
        # changed
        interp_pred = np.empty((0, time_points.shape[0]))


        for patient in pred.columns:
            #interp_patient = np.interp(torch.arange(0.5, 30, 1), # changed
            interp_patient = np.interp(time_points,
...
class Evaluation(_BaseEvaluation):
    ...
    def _collect_patient_predictions(self, time_points=None):
        ...
        if self.type == 'MultiSurv':
            ...
            times, events, pred = self.ev._predict(time_points=time_points)

            print()

            for i, patient in enumerate(pids):
                # change [i] to [patient] for time, event

                patient_data[patient] = {'time': times[patient],
                                         'event': events[patient],
    ...
    def _compute_pycox_metrics(self,  data, time_points=None, # changed
                               drop_last_times=25):
    ...
    def compute_metrics(self, time_points=None):
        ...
        if self.patient_predictions is None:
            # Get all patient labels and predictions
            self.patient_predictions = self._collect_patient_predictions(time_points=time_points)
    ...
    def run_bootstrap(self, n=1000, time_points=None):
        ...
        time_points: torch.Tensor
            Time tpoints of the predictions.
        Returns
        -------
