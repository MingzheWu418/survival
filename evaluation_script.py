...
if __name__ == '__main__':
    ...
    print(args.cancer_type, args.max_iter)
    ...
    for preproc_id in PREPROC_ID_TO_KWARGS:
        ...
        for seed in tqdm(SEED_LIST):
            ...
            if args.model_name in ["MLP", "Linear"]:
                model.train(train_ds, valid_ds, max_iter=args.max_iter)
            else:
            ...
            for split, ds in zip(['train', 'valid', 'test'], [train_ds, valid_ds, test_ds]):
                if len(ds.expr_data) == 0:
                    ...
                    # Use linear or MLP model to predict each patient is noisy or clean
            ...
            # ''' This is the obsolete gmm implementation'''
            ...
            for split, ds in zip(['train', 'valid', 'test'], [train_ds, valid_ds, test_ds]):
                ...
                if empty_set[split] == 1:
                    ...
                    if args.model_name in ["Linear", "MLP"]:
                        ...
                        args.model_name, preproc_id, str(args.max_iter))), 'wb') as f:
                    ...
                    if args.model_name in ["Linear", "MLP"]:
                        ...
                        args.model_name, str(args.max_iter))))
                    else:
